resource "aws_cloudfront_distribution" "web_cdn" {
    origin {
        domain_name = "web-${var.environment}.${var.zone_name}" #web-dev.neelareddy.store
    ...
    aliases = ["web-${var.common_tags.Component}.${var.zone_name}"] #web-cdn.neelareddy.store
...
module "records" {
    ...
    records = [
        {
            name = "web-cdn" #web-cdn.neelareddy.store
            type = "A"
